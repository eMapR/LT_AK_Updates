...
def calc_strata_sizes(ar, nodata_mask, sample_size, bins, scheme='proportional', bin_scale=1, zero_inflation=0):
    ...
    if scheme == 'proportional':
        ...
        counts_mode = stats.mode(counts, axis = None)
        print 'counts_mode is ', counts_mode
        if int(counts_mode.mode[0])==0: 
            print 'entered if statement'
            n_per_bin = counts
            scaled_percents = counts.astype(np.float)/n_pixels
            ...
            print 'bin percents are: ', bin_percents
