...
def main(params, model_type='classifier', max_depth=None, max_features=None):
    ...
    if model_type.lower() == 'classifier':
        print 'Training random forest with classifier algorithm...'
        model_func = forest.train_rf_classifier
    elif model_type.lower().strip() == 'zeroinflated':
        print 'Training STEM with zeroinflated regression algorithm...'
        model_func = forest.train_rf_zeroinflated
    else:
        print 'Training random forest with regressor algorithm...'
        model_func = forest.train_rf_regressor
        
    rf_model = model_func(x_train, y_train,
                          ntrees=n_trees,
                          njobs=n_jobs,
                          max_depth=max_depth,
    ...
    if model_type.lower().strip() != 'zeroinflated':
    ...
    rf_path = os.path.join(out_dir, 'rf_model_%s.pkl' % stamp)
